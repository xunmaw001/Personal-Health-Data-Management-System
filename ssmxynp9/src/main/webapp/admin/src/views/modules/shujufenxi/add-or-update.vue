<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="分析结果" prop="fenxijieguo">
          <el-select v-model="ruleForm.fenxijieguo" placeholder="请选择分析结果">
            <el-option
                v-for="(item,index) in fenxijieguoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="分析结果" prop="fenxijieguo">
	      <el-input v-model="ruleForm.fenxijieguo"
                placeholder="分析结果" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.keguantuxiang" label="客观图像" prop="keguantuxiang">
          <file-upload
          tip="点击上传客观图像"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.keguantuxiang?ruleForm.keguantuxiang:''"
          @change="keguantuxiangUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.keguantuxiang" label="客观图像" prop="keguantuxiang">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.keguantuxiang.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="健康建议" prop="jiankangjianyi">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.jiankangjianyi" label="健康建议" prop="jiankangjianyi">
                    <span v-html="ruleForm.jiankangjianyi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.jiankangjianyi = this.ruleForm.jiankangjianyi.replace(reg,'../../../ssmxynp9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    keguantuxiangUploadChange(fileUrls) {
	this.ruleForm.keguantuxiang = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
