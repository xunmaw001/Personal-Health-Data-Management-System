      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="饮食习惯" prop="yinshixiguan">
          <el-input v-model="ruleForm.yinshixiguan" 
              placeholder="饮食习惯" clearable  :readonly="ro.yinshixiguan"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="饮食习惯" prop="yinshixiguan">
              <el-input v-model="ruleForm.yinshixiguan" 
                placeholder="饮食习惯" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="运动习惯" prop="yundongxiguan">
          <el-select v-model="ruleForm.yundongxiguan" placeholder="请选择运动习惯">
            <el-option
                v-for="(item,index) in yundongxiguanOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="运动习惯" prop="yundongxiguan">
	      <el-input v-model="ruleForm.yundongxiguan"
                placeholder="运动习惯" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  data() {
	...
	clicknum : false,
      ...
      rules: {
          ...
          yinshixiguan: [
                { required: true, message: '饮食习惯不能为空', trigger: 'blur' },
          ],
          yundongxiguan: [
  ...
  methods: {
    ...
    yonghuzhanghaoChange () {
      this.$http({
        ...
        if (data && data.code === 0) {
              if(data.data.yonghuming){
                this.ruleForm.yonghuming = data.data.yonghuming
              }
    ...
    info(id) {
      this.$http({
        url: `jiankangxinxigongxiang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `jiankangxinxigongxiang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.jiankangxinxigongxiangCrossAddOrUpdateFlag = false;
                  this.parent.search();
