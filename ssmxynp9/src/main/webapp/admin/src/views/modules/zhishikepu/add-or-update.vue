<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="视频介绍" prop="shipinjieshao">
          <file-upload
          tip="点击上传视频介绍"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.shipinjieshao" label="视频介绍" prop="shipinjieshao">
            <el-button type="text" size="small" @click="download(ruleForm.shipinjieshao)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xiangxineirong = this.ruleForm.xiangxineirong.replace(reg,'../../../ssmxynp9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhishikepu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhishikepuCrossAddOrUpdateFlag = false;
                  this.parent.search();
