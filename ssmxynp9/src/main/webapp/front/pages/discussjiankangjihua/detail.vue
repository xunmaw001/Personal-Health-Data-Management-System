...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussjiankangjihua');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discussjiankangjihua');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discussjiankangjihua', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussjiankangjihua', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussjiankangjihua/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussjiankangjihua', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussjiankangjihua', this.detail);
				this.$utils.msg('审核成功');
